...
einen Stream von Zeilen — welche Terminaloperation zaehlt? Und frag dich: Wer
...
try-with-resources-Blocks oeffnen, im Block `count()` aufrufen und das Ergebnis
direkt zurueckgeben. Ein `return` innerhalb von `try (...)` ist voellig in
Ordnung — der Stream wird trotzdem geschlossen.

Die Randfaelle (leere Datei -> `0`, letzte Zeile ohne `\n` -> zaehlt trotzdem)
...
Abschnitt 11.2 (Schreiben) und 11.3 (Zeichensaetze). Frag dich: Welcher
...
legt `writeString` die Datei an oder ueberschreibt sie — genau das verlangt der
Test "ueberschreibt".

Denkfallen:

- `Files.write(pfad, zeilen, ...)` haengt den **Plattform**-Zeilentrenner an —
  unter Linux `\n`, unter Windows `\r\n`. Dann sind die Tests nur auf manchen
  Systemen gruen.
- `String.join("\n", zeilen) + "\n"` klingt praktisch, liefert fuer die leere
  Liste aber `"\n"` statt `""`. Der Test "leere Liste" faellt dann durch.
- Den Zeichensatz explizit angeben — der Test schreibt Umlaute hin und zurueck.
...
Wie in Aufgabe 1 den Stream in try-with-resources oeffnen, dann
...
dann ein leerer String. Entweder erst strippen und dann auf leer pruefen, oder
...
werden. Frag dich: Was gibst du im Erfolgsfall zurueck, was im Fehlerfall?
...
`Optional.empty()` zurueckgeben. Eine fehlende Datei meldet Java als
`NoSuchFileException` — das ist eine Unterklasse von `IOException`, wird also
mitgefangen.

Fang nicht `Exception` ab: Das wuerde auch echte Programmierfehler
...
**Zeilennummer**? Und welche zwei Dinge koennen bei einer Zeile schiefgehen?
...
- `Files.readAllLines(pfad, StandardCharsets.UTF_8)` liefert eine `List<String>`.
  ...
  Kopfzeile (Index 0) uebersprungen. Die Zeilennummer in der Datei ist dann
  `i + 1`.
- Leere Zeilen mit `isBlank()` ueberspringen (`continue`). Sie zaehlen trotzdem
  mit, weil der Index weiterlaeuft — das prueft der Test "Zeile 4".
- `split(";", -1)`: Nur mit `-1` hat `"temp;21.5;"` drei Felder. Ohne es fallen
  leere Endfelder weg, die Zeile saehe gueltig aus.
- Pruefe `felder.length != 2`, dann `Double.parseDouble(...)`. Dessen
  `NumberFormatException` faengst du und wirfst eine eigene
  `IllegalArgumentException` mit der richtigen Nachricht (gern mit `e` als
  Ursache).

Randfaelle: Eine leere Datei ergibt eine leere Liste — die Schleife ab `1` laeuft
dann gar nicht. Und: `NumberFormatException` *ist* schon eine
`IllegalArgumentException`. Laesst du sie einfach durch, stimmt der Typ, aber
die Nachricht nicht, und der Test wird rot. Die Nachricht enthaelt die
...
(`Locale.ROOT` beim Formatieren). Frag dich: Wie bekommst du die Eintraege
**alphabetisch**, wenn die uebergebene Map (`Map.of(...)`) keine Reihenfolge
garantiert?
...
`new TreeMap<>(durchschnitte)` sortiert nach Schluessel. Ueber deren
...
aussagekraeftige Exceptions wie `NoSuchFileException` oder
`AccessDeniedException`. Dazu kommen bequeme Methoden (`readString`,
`writeString`, `lines`), UTF-8 als Standard und plattformunabhaengige Pfade
ueber `Path.of(...)`. `File` brauchst du nur noch, wenn eine alte Bibliothek es
...
`Files.lines` liest **faul**: Die Datei bleibt geoeffnet, solange der Stream
lebt, damit er weitere Zeilen nachladen kann. Ein normaler Stream ueber eine
`List` haelt keine Ressource, `Files.lines` dagegen einen Dateideskriptor des
...
Dateien an, bis "too many open files" kommt (und unter Windows laesst sich die
Datei so lange nicht loeschen).

</details>

<details><summary>Was passiert ohne expliziten Zeichensatz — und warum ist das heute weniger schlimm als frueher?</summary>

Die `Files`-Methoden nehmen ohne Angabe **immer UTF-8**. Aeltere APIs wie
...
die Absicht und schuetzt auf aelteren JVMs.
...
`split` **leere Felder am Ende**, ein negativer Wert behaelt sie. Das ist fuer
CSV wichtig, weil ein leeres letztes Feld sonst einfach verschwindet und eine
kaputte Zeile gueltig aussieht:
