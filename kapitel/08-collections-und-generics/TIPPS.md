...
(Abschnitt 8.8, "Type Erasure") zur Laufzeit überhaupt mit `instanceof` prüfen?
...
(Abschnitt 8.7). In einem `TreeSet` gälten Anna(34) und Anna(99) als ein und
dasselbe Element.
...
- Deshalb: erst `strip()`, dann auf leer prüfen und ggf. die leere Map
  zurückgeben, erst dann trennen.

Kleinschreiben mit `toLowerCase(Locale.ROOT)`. Zum Zählen `merge(wort, 1, Integer::sum)`:
Ist das Wort neu, wird 1 eingetragen, sonst wird die alte Zahl plus 1 gespeichert.
`\\s` erfasst übrigens auch Zeilenumbrüche, deshalb klappt auch der Test mit `\n`.
...
`Map.Entry.comparingByKey()`. **Die Falle:** Hängst du direkt `.reversed()` an,
...
ein `List.of(...)`, gäbe es sogar eine `UnsupportedOperationException`. Also:
...
kann der Compiler den Typ von `p` nicht ableiten (für ihn ist `p` ein `Object`)
...
- `computeIfAbsent(schluessel, k -> new ArrayList<>())` liefert die Liste zu
  ...
  Ergebnis kannst du direkt `.add(wort)` hängen.
...
<details><summary>Was passiert, wenn du <code>equals</code> überschreibst und <code>hashCode</code> vergisst?</summary>
...
Die for-each-Schleife benutzt intern einen `Iterator`. Die Liste zählt jede
strukturelle Änderung mit, und der Iterator prüft bei jedem `next()`, ob sich
...
Wäre es eines, könntest du über die `Number`-Sicht etwas Falsches hineinlegen:
