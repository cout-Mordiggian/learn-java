...
Abschnitt 3.4, `StringBuilder` — die Liste der nuetzlichen Methoden am Ende.
...
`String` selbst hat keine Umdreh-Methode (unveraenderlich, Abschnitt 3.1), aber
`StringBuilder` hat `reverse()`. Ein `StringBuilder` laesst sich direkt mit einem
`String` im Konstruktor erzeugen. Denkfalle: `reverse()` gibt einen
`StringBuilder` zurueck, keinen `String` — du brauchst am Ende noch `toString()`,
sonst kompiliert die Rueckgabe nicht. Leerer String und ein einzelnes Zeichen
...
pruefst du dann, ob es vorwaerts und rueckwaerts gleich ist?

</details>

<details><summary>Tipp 2 — Ansatz</summary>

Schritt 1: Laufe mit `for (char c : text.toCharArray())` ueber alle Zeichen,
behalte nur die, fuer die `Character.isLetterOrDigit(c)` gilt, und haenge sie
...
weil `reverse()` denselben Builder veraendert und zurueckgibt. Oder zwei Indizes
...
die aeusseren Zeichen vergleichst. Der leere String ist ein Palindrom.
...
Abschnitt 3.3: `strip()`, `isEmpty()` und `split` mit regulaerem Ausdruck
...
und Zeilenumbruch ab (Test `"a\tb\nc"`) und fasst Mehrfachabstaende zusammen.
...
Array mit **einem** leeren String, also Laenge `1`. Den Fall "nach dem Strippen
...
Abschnitt 2.4. Frag dich: Welchen Startwert hat dein "bisher groesster Wert",
...
Denkfalle: Startest du mit `0`, liefert `maximum({-5, -2, -9})` faelschlich `0`
— eine Zahl, die gar nicht im Array steht. Starte stattdessen mit einem Wert,
der garantiert dazugehoert: dem ersten Element (die Aufgabe sichert zu, dass das
Array nicht leer ist). Dann laeufst du ueber die restlichen Elemente und
ersetzt den Kandidaten, sobald du einen groesseren findest. Die Tests pruefen
...
Kapitel 1, Abschnitt 1.4: "Ganzzahldivision" **und** "Ueberlauf ohne Warnung" —
...
Summe sammeln, durch `werte.length` teilen. Drei Fallen, alle vom Test geprueft:
...
- **Ganzzahldivision:** `summe / werte.length` mit zwei Ganzzahlen ergibt bei
  ...
  spaet — der Cast muss auf einen **Operanden**, nicht auf das Ergebnis.
- **Ueberlauf:** `Integer.MAX_VALUE + Integer.MAX_VALUE` passt nicht in `int`
...
Abschnitt 3.6: "Nuetzliche Helfer in `java.util.Arrays`" und "Arrays sind
Referenztypen". Frag dich: Veraendert `Arrays.sort` das Array, das du ihm gibst
— und was bedeutet das fuer das Original des Aufrufers?
...
Original, und der Test "Original ist unveraendert" schlaegt fehl. Zweite Falle:
`Arrays.sort` gibt nichts zurueck (`void`), `return Arrays.sort(kopie);`
...
`matrix[0].length` die Spaltenzahl. Frag dich: Welche Groesse hat das Ergebnis
fuer eine 2x3-Matrix — und wo landet das Element aus Zeile `i`, Spalte `j`?
...
verschachtelte Schleifen ueber die Eingabe; beim Schreiben ins Ergebnis tauschst
du einfach die beiden Indizes. Randfall "leere Matrix": `matrix[0]` gibt es
dann nicht — der Zugriff wirft eine Exception. Pruefe `matrix.length == 0`
vorher und gib ein leeres `int[][]` zurueck (z. B. `new int[0][]`).
...
Bei `n` Teilen gibt es `n - 1` Trenner. Am saubersten: Haenge den Trenner
**vor** jedem Element an, ausser vor dem ersten (Index `0`). Dafuer brauchst du
den Index, also keine `for-each`-Schleife. Die Alternative "immer Trenner
anhaengen und am Ende das letzte Zeichen abschneiden" hat zwei Fallen: Bei
leerem Array gibt es nichts abzuschneiden, und beim mehrzeichigen Trenner
`", "` entfernst du nur eines der beiden Zeichen — genau das prueft der Test.
`String.join` selbst ist hier natuerlich tabu.
...
<details><summary>Warum aendert `s.toUpperCase();` (ohne Zuweisung) nichts?</summary>

`String` ist unveraenderlich: `toUpperCase()` veraendert das vorhandene Objekt
nicht, sondern erzeugt ein **neues** `String`-Objekt mit dem Ergebnis und gibt
es zurueck. Ohne Zuweisung wird dieses neue Objekt einfach weggeworfen, und `s`
zeigt weiter auf den alten Text. Richtig ist `s = s.toUpperCase();` — dann zeigt
`s` auf den neuen String. Das gilt fuer alle "aendernden" String-Methoden wie
...
zeigen `a = "hallo"` und `b = "hallo"` tatsaechlich auf dasselbe Objekt. Ein
...
also `false`. Deshalb immer `equals` fuer den Inhalt.

</details>

<details><summary>Warum gibt `System.out.println(intArray)` Kauderwelsch aus?</summary>

Arrays ueberschreiben `toString()` nicht, also wird die Standardversion aus
...
Array, eine Aenderung ueber `b[0] = 99` sieht man auch in `a[0]`.
`a.clone()` legt ein **neues** Array mit denselben Werten an; danach sind beide
unabhaengig, und `a == b` ist `false`. Die Kopie ist flach: Bei einem Array von
...
Erzeugung gesetzt wird und sich nie aendert; Felder liest man ohne Klammern.
`String` ist dagegen eine normale Klasse, und `length()` ist eine **Methode**,
die die Laenge aus den internen Daten des Strings ermittelt; Methoden ruft man
